...
const Purchase: FunctionComponent<PurchaseProps> = ({
  ...
  const {
    ...
    screen,
    setScreen,
  } = usePurchase(nft, setNft, address, publicClient);
  const {
    rechargeLoading,
    handleRecharge,
    setRechargeAmount,
    ...
    approvedRecharge,
  } = useAgentRecharge(nft?.agentIds, publicClient, address, nft?.id);
  const {
    handlePost,
    postLoading,
    interactionsLoading,
    ...
    setCommentQuote,
    success,
  } = useInteractions(setNft, nft, handlePosts);
  return (
    <div
            ...
            {screen < 1 ? (
              <>
                ...
                <div className="relative w-full font-start justify-between items-center flex flex-wrap sm:flex-nowrap flex-row gap-3">
                  ...
                  <div className="relative w-fit h-fit justify-end flex text-sm">
                    ...
                    )?.toFixed(2)}{" "}
                    {
                      TOKENS?.find(
                        (tok) =>
                          collectData?.token == tok.contract?.toLowerCase()
                      )?.symbol
                    }
                  </div>
                </div>
                        ...
                        Number(
                          ...
                          Number(
                            context?.tokenThresholds?.find(
                              (t) =>
                                t?.token?.toLowerCase() == collectData?.token
                            )?.threshold
                          )) ||
                      (Number(nft?.agentIds?.length || 0) > 0 &&
                        Number(nft?.amount || 0) > 2 &&
                ...
                <div className="relative w-full h-full flex flex-col gap-3 items-start justify-between">
                  <div
                    className={`relative w-full h-fit flex py-4 overflow-y-scroll max-h-28`}
                  >
                    <div
                      className={`py-1 h-fit flex relative items-start justify-start text-left text-sm font-nerd break-all w-full`}
                  ...
                  <div className="relative w-full h-fit flex-col gap-1 flex py-1 items-start justify-start">
                    <div className="relative w-fit h-fit text-right flex text-sm font-nerd">
                      {nft?.isAgent ? "Agent Minted" : "Hand Minted"}
                    </div>
                      ...
                      return (
                        <div
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            window.open(
                              `https://explorer.lens.xyz/tx/${collector?.transactionHash}`
                            );
                          }}
                        >
                          {collector?.name ? (
                          ...
                          <div className="relative w-full h-fit flex flex-row gap-2 justify-between items-center md:flex-nowrap flex-wrap">
                            ...
                            >
                              <div className="relative w-8 h-8 flex rounded-sm">
                                <Image
                                  src={handleProfilePicture(agent?.cover)}
                                  alt="pfp"
                                  draggable={false}
                                  layout="fill"
